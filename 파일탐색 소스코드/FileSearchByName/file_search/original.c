...
DWORD WINAPI Searchthread(LPVOID lpParam) 
{
    ...
    search_execute(); // search_execute 함수를 호출하여 경로에서 파일을 검색
...
//         for (int i = 0; i < NUM_THREADS && i < drive_count; ++i) 
//         {
//             threads[i] = CreateThread(NULL, 0, Searchthread, drive[i], 0, &threadIds[i]); // 스레드 생성
